...
function check_reinstall_mtg()
	-- used to be in aperosengine.conf
	...
	for _, world in ipairs(worlds) do
		if world.gameid == "aperosvoxel" then
...
local function get_formspec(dialogdata)
	local markup = table.concat({
		"<big>", fgettext("AperosVoxel is no longer installed by default"), "</big>\n",
		fgettext("Install AperosVoxel from our official Github.")
	})

	return table.concat({
		"formspec_version[6]",
		"size[12.8,7]",
		"hypertext[0.375,0.375;12.05,5.2;text;", aperosengine.formspec_escape(markup), "]",
