...
core.builtin_auth_handler = {
	...
	set_privileges = function(name, privileges)
		...
		for priv, value in pairs(privileges) do
			...
			if value == false then
				core.log('deprecated', "`false` value given to `aperosengine.set_player_privs`, "..
						"this is almost certainly a bug, "..
						"granting a privilege rather than revoking it")
			elseif value ~= true then
				core.log('deprecated', "non-`true` value given to `aperosengine.set_player_privs`")
			end
			-- Run grant callbacks
...
function core.change_player_privs(name, changes)
	...
	for priv, change in pairs(changes) do
		...
		elseif change == false then
			...
			error("non-bool value given to `aperosengine.change_player_privs`")
		end
	end
