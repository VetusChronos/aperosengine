...
function aperosengine_field_helper(lentype, name, abbr)
...
aperosengine_client_commands = {}
aperosengine_client_obsolete = {}
...
do
	...
	local f_player_namelen, f_player_name =
		aperosengine_field_helper("uint16", abbr.."player_name", "Player Name")

	aperosengine_client_commands[0x02] = {
		"INIT",                            -- Command name
		...
		function(buffer, pinfo, tree, t)   -- Dissector function [optional]
			...
			aperosengine_decode_helper_ascii(buffer, t, "uint16", 9, f_player_namelen, f_player_name)
		end
	}
end
...
aperosengine_client_commands[0x10] = { "INIT_LEGACY", 53 }
aperosengine_client_obsolete[0x10] = true
...
do
	...
	aperosengine_client_commands[0x11] = {
		...
		function(buffer, pinfo, tree, t)
			aperosengine_decode_helper_ascii(buffer, t, "uint16", 2, f_langlen, f_lang)
		end
	}
end
...
aperosengine_client_commands[0x17] = { "MODCHANNEL_JOIN", 2 }

-- TOSERVER_MODCHANNEL_LEAVE

aperosengine_client_commands[0x18] = { "MODCHANNEL_LEAVE", 2 }

-- TOSERVER_MODCHANNEL_MSG

aperosengine_client_commands[0x19] = { "MODCHANNEL_MSG", 2 }

-- TOSERVER_GETBLOCK (obsolete)

aperosengine_client_commands[0x20] = { "GETBLOCK", 2 }
aperosengine_client_obsolete[0x20] = true

-- TOSERVER_ADDNODE (obsolete)

aperosengine_client_commands[0x21] = { "ADDNODE", 2 }
aperosengine_client_obsolete[0x21] = true

-- TOSERVER_REMOVENODE (obsolete)

aperosengine_client_commands[0x22] = { "REMOVENODE", 2 }
aperosengine_client_obsolete[0x22] = true
...
do
	...
	aperosengine_client_commands[0x23] = {
		"PLAYERPOS", 34,
...
do
	...
	aperosengine_client_commands[0x24] = {
		...
		function(buffer, pinfo, tree, t)
			...
			if aperosengine_check_length(buffer, 3 + 6*count, t) then
...
aperosengine_client_commands[0x26] = { "ADDNODE_FROM_INVENTORY", 2 }
aperosengine_client_obsolete[0x26] = true

-- TOSERVER_CLICK_OBJECT (obsolete)

aperosengine_client_commands[0x27] = { "CLICK_OBJECT", 2 }
aperosengine_client_obsolete[0x27] = true

-- TOSERVER_GROUND_ACTION (obsolete)

aperosengine_client_commands[0x28] = { "GROUND_ACTION", 2 }
aperosengine_client_obsolete[0x28] = true

-- TOSERVER_RELEASE (obsolete)

aperosengine_client_commands[0x29] = { "RELEASE", 2 }
aperosengine_client_obsolete[0x29] = true

-- TOSERVER_SIGNTEXT (obsolete)

aperosengine_client_commands[0x30] = { "SIGNTEXT", 2 }
aperosengine_client_obsolete[0x30] = true
...
do
	...
	aperosengine_client_commands[0x32] = {
		...
		function(buffer, pinfo, tree, t)
			...
			if aperosengine_check_length(buffer, 4 + textlen*2, t) then
...
aperosengine_client_commands[0x33] = { "SIGNNODETEXT", 2 }
aperosengine_client_obsolete[0x33] = true


-- TOSERVER_CLICK_ACTIVEOBJECT (obsolete)

aperosengine_client_commands[0x34] = { "CLICK_ACTIVEOBJECT", 2 }
aperosengine_client_obsolete[0x34] = true
...
aperosengine_client_commands[0x36] = { "CLICK_ACTIVEOBJECT", 2 }
aperosengine_client_obsolete[0x36] = true
...
aperosengine_client_commands[0x38] = { "RESPAWN", 2 }
...
do
	...
	aperosengine_client_commands[0x39] = {
		"INTERACT", 11,
		{ f_action,
		  f_item,
		  f_plen,
		...
		function(buffer, pinfo, tree, t)
			...
			if aperosengine_check_length(buffer, 9 + plen, t) then
...
aperosengine_client_commands[0x3a] = { "REMOVED_SOUNDS", 2 }
aperosengine_client_commands[0x3b] = { "NODEMETA_FIELDS", 2 }
aperosengine_client_commands[0x3c] = { "INVENTORY_FIELDS", 2 }
aperosengine_client_commands[0x40] = { "REQUEST_MEDIA", 2 }
aperosengine_client_commands[0x41] = { "RECEIVED_MEDIA", 2 }

-- TOSERVER_BREATH (obsolete)

aperosengine_client_commands[0x42] = { "BREATH", 2 }
aperosengine_client_obsolete[0x42] = true
...
do
	...
	local f_versionlen, f_version =
		aperosengine_field_helper("uint16", abbr.."version", "Full Version String")
	local f_formspec_ver = ProtoField.uint16(abbr.."formspec_version",
		"Formspec API version")

	aperosengine_client_commands[0x43] = {
		"CLIENT_READY",
		8,
		...
		function(buffer, pinfo, tree, t)
			...
			local off = aperosengine_decode_helper_ascii(buffer, t, "uint16", 6,
				f_versionlen, f_version)
			if off and aperosengine_check_length(buffer, off + 2, t) then
...
aperosengine_client_commands[0x50] = { "FIRST_SRP", 2 }
aperosengine_client_commands[0x51] = { "SRP_BYTES_A", 2 }
aperosengine_client_commands[0x52] = { "SRP_BYTES_M", 2 }
...
aperosengine_server_commands = {}
aperosengine_server_obsolete = {}
...
do
	...
	local f_legacy_namelen, f_legacy_name = aperosengine_field_helper("uint16",
		abbr.."legacy_name", "Legacy player name for hashing")

	aperosengine_server_commands[0x02] = {
		"HELLO",
		13,
		...
		function(buffer, pinfo, tree, t)
			...
			aperosengine_decode_helper_ascii(buffer, t, "uint16", 11, f_legacy_namelen, f_legacy_name)
		end
	}
end
...
do
	...
	aperosengine_server_commands[0x03] = {
		"AUTH_ACCEPT",
		30,
...
aperosengine_server_commands[0x04] = {"ACCEPT_SUDO_MODE", 2}
aperosengine_server_commands[0x05] = {"DENY_SUDO_MODE", 2}
aperosengine_server_commands[0x0A] = {"ACCESS_DENIED", 2}

-- TOCLIENT_INIT (obsolete)

aperosengine_server_commands[0x10] = { "INIT", 2 }
aperosengine_server_obsolete[0x10] = true
...
aperosengine_server_commands[0x23] = { "PLAYERPOS", 2 }
aperosengine_server_obsolete[0x23] = true

-- TOCLIENT_PLAYERINFO (obsolete)

aperosengine_server_commands[0x24] = { "PLAYERINFO", 2 }
aperosengine_server_obsolete[0x24] = true

-- TOCLIENT_OPT_BLOCK_NOT_FOUND (obsolete)

aperosengine_server_commands[0x25] = { "OPT_BLOCK_NOT_FOUND", 2 }
aperosengine_server_obsolete[0x25] = true

-- TOCLIENT_SECTORMETA (obsolete)

aperosengine_server_commands[0x26] = { "SECTORMETA", 2 }
aperosengine_server_obsolete[0x26] = true
...
aperosengine_server_commands[0x28] = { "OBJECTDATA", 2 }
aperosengine_server_obsolete[0x28] = true
...
aperosengine_server_commands[0x2a] = { "CSM_RESTRICTION_FLAGS", 2 }

-- TOCLIENT_PLAYER_SPEED

aperosengine_server_commands[0x2b] = { "PLAYER_SPEED", 2 }
...
do
	...
	local f_senderlen, f_sender = aperosengine_field_helper("uint16", abbr.."sender",
		"Message sender")
	local f_messagelen, f_message = aperosengine_field_helper("uint16", abbr:sub(1,-2),
		"Message")

	aperosengine_server_commands[0x2f] = {
		"CHAT_MESSAGE", 8,
		...
		function(buffer, pinfo, tree, t)
			...
			local off = 4
			off = aperosengine_decode_helper_utf16(buffer, t, "uint16", off, f_senderlen, f_sender)
			if off then
				off = aperosengine_decode_helper_utf16(buffer, t, "uint16", off, f_messagelen, f_message)
			end
		end
	}
end
...
aperosengine_server_commands[0x30] = { "CHAT_MESSAGE_OLD", 2 }
aperosengine_server_obsolete[0x30] = true
...
do
	...
	aperosengine_server_commands[0x31] = {
		...
		function(buffer, pinfo, tree, t)
			...
			if not aperosengine_check_length(buffer, added_count_pos + 2, t) then
				return
			end
			...
			for index = 0, added_count - 1 do
				if not aperosengine_check_length(buffer, pos + 7, t) then
					return
				end

				local init_length = buffer(pos + 3, 4):uint()
				if not aperosengine_check_length(buffer, pos + 7 + init_length, t) then
					return
				end
	...
	aperosengine_server_commands[0x32] = {
		...
		function(buffer, pinfo, tree, t)
			...
			while pos < buffer:len() do
				if not aperosengine_check_length(buffer, pos + 4, t) then
					return
				end
				message_length = buffer(pos + 2, 2):uint()
				if not aperosengine_check_length(buffer, pos + 4 + message_length, t) then
					return
				end
				count = count + 1
...
do
	...
	aperosengine_server_commands[0x34] = {
		"MOVE_PLAYER", 22,
...
do
	...
	aperosengine_server_commands[0x35] = {
		...
		function(buffer, pinfo, tree, t)
			...
			if aperosengine_check_length(buffer, 4 + reason_length * 2, t) then
				t:add(f_reason, aperosengine_convert_utf16(buffer(4, reason_length * 2), "Converted reason message"))
			end
		end
	}
end
...
aperosengine_server_commands[0x36] = { "FOV", 2 }
...
aperosengine_server_commands[0x38] = {"MEDIA", 2}

-- TOCLIENT_TOOLDEF (obsolete)

aperosengine_server_commands[0x39] = {"TOOLDEF", 2}
aperosengine_server_obsolete[0x39] = true

-- TOCLIENT_NODEDEF

aperosengine_server_commands[0x3a] = {"NODEDEF", 2}

-- TOCLIENT_CRAFTITEMDEF (obsolete)

aperosengine_server_commands[0x3b] = {"CRAFTITEMDEF", 2}
aperosengine_server_obsolete[0x3b] = true

-- ...

aperosengine_server_commands[0x3c] = {"ANNOUNCE_MEDIA", 2}
aperosengine_server_commands[0x3d] = {"ITEMDEF", 2}
aperosengine_server_commands[0x3f] = {"PLAY_SOUND", 2}
aperosengine_server_commands[0x40] = {"STOP_SOUND", 2}
aperosengine_server_commands[0x41] = {"PRIVILEGES", 2}
aperosengine_server_commands[0x42] = {"INVENTORY_FORMSPEC", 2}
aperosengine_server_commands[0x43] = {"DETACHED_INVENTORY", 2}
aperosengine_server_commands[0x44] = {"SHOW_FORMSPEC", 2}
aperosengine_server_commands[0x45] = {"MOVEMENT", 2}
aperosengine_server_commands[0x46] = {"SPAWN_PARTICLE", 2}
aperosengine_server_commands[0x47] = {"ADD_PARTICLE_SPAWNER", 2}

-- TOCLIENT_DELETE_PARTICLESPAWNER_LEGACY (obsolete)

aperosengine_server_commands[0x48] = {"DELETE_PARTICLESPAWNER_LEGACY", 2}
aperosengine_server_obsolete[0x48] = true

-- ...

aperosengine_server_commands[0x49] = {"HUDADD", 2}
aperosengine_server_commands[0x4a] = {"HUDRM", 2}
aperosengine_server_commands[0x4b] = {"HUDCHANGE", 2}
aperosengine_server_commands[0x4c] = {"HUD_SET_FLAGS", 2}
aperosengine_server_commands[0x4d] = {"HUD_SET_PARAM", 2}
aperosengine_server_commands[0x4e] = {"BREATH", 2}
aperosengine_server_commands[0x4f] = {"SET_SKY", 2}
aperosengine_server_commands[0x50] = {"OVERRIDE_DAY_NIGHT_RATIO", 2}
aperosengine_server_commands[0x51] = {"LOCAL_PLAYER_ANIMATIONS", 2}
aperosengine_server_commands[0x52] = {"EYE_OFFSET", 2}
aperosengine_server_commands[0x53] = {"DELETE_PARTICLESPAWNER", 2}
aperosengine_server_commands[0x54] = {"CLOUD_PARAMS", 2}
aperosengine_server_commands[0x55] = {"FADE_SOUND", 2}
...
do
	...
	aperosengine_server_commands[0x56] = {
		"UPDATE_PLAYER_LIST",
		5,
		...
		function(buffer, pinfo, tree, t)
			...
			local off = 5
			for i = 1, count do
				if not aperosengine_check_length(buffer, off + 2, t) then
					return
				end
				off = aperosengine_decode_helper_ascii(buffer, t, "uint16", off, nil, f_name)
				if not off then
					return
				end
			end
...
aperosengine_server_commands[0x57] = {"MODCHANNEL_MSG", 2}
aperosengine_server_commands[0x58] = {"MODCHANNEL_SIGNAL", 2}
aperosengine_server_commands[0x59] = {"NODEMETA_CHANGED", 2}
aperosengine_server_commands[0x5a] = {"SET_SUN", 2}
aperosengine_server_commands[0x5b] = {"SET_MOON", 2}
aperosengine_server_commands[0x5c] = {"SET_STARS", 2}
aperosengine_server_commands[0x60] = {"SRP_BYTES_S_B", 2}
aperosengine_server_commands[0x61] = {"FORMSPEC_PREPEND", 2}
...
function aperosengine_define_client_or_server_proto(is_client)
	...
	if is_client then
		...
		commands = aperosengine_server_commands  -- defined in Part 3
		obsolete = aperosengine_server_obsolete  -- defined in Part 3
	end
	...
	function proto.dissector(buffer, pinfo, tree)
		...
		elseif aperosengine_check_length(buffer, 2, t) then
			...
			if command_info == nil then
				...
				if aperosengine_check_length(buffer, command_min_length, t) then
...
function aperosengine_check_length(tvb, min_len, t)
...
-- t_textlen, t_text should be the ProtoFields created by aperosengine_field_helper
--   alternatively t_text can be a ProtoField.string and t_textlen can be nil
-- lentype must be the type of the length field (as passed to aperosengine_field_helper)
-- returns nil if length check failed
function aperosengine_decode_helper_ascii(tvb, t, lentype, offset, f_textlen, f_text)
	...
	if aperosengine_check_length(tvb, offset + n + textlen, t) then
...
-- (see aperosengine_decode_helper_ascii)
function aperosengine_decode_helper_utf16(tvb, t, lentype, offset, f_textlen, f_text)
	local n = ({uint16 = 2, uint32 = 4})[lentype]
	assert(n)
