...
aperosengine=$dir/../bin/aperosengine
testspath=$dir/../tests
...
[ -e "$aperosengine" ] || { echo "executable $aperosengine missing"; exit 1; }
...
"$aperosengine" --debugger --server --config "$conf_server" --world "$worldpath" --gameid $gameid 2>&1 \
	| sed -u 's/^/(server) /' | tee -a "$testspath/log.txt" &
waitfor "$worldpath/startup"

echo "Starting client"
"$aperosengine" --debugger --config "$conf_client1" --go --address 127.0.0.1 2>&1 \
