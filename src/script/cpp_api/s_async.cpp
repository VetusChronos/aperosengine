...
void AsyncEngine::stepAutoscale()
{
	...
	if (autoscaleTimer && porting::getTimeMs() >= autoscaleTimer) {
		...
		infostream << "AsyncEngine: " << n << " jobs were still waiting after 1s" << '\n';
...
bool AsyncEngine::prepareEnvironment(lua_State* L, int top)
{
	...
	if (server) {
		...
		try {
			...
			errorstream << "Failed to load mod script inside async environment." << '\n';
...
void* AsyncWorkerThread::run()
{
	...
	while (!stopRequested()) {
		...
		if (luaL_loadbuffer(L, j.function.data(), j.function.size(), "=(async)")) {
			errorstream << "ASYNC WORKER: Unable to deserialize function" << '\n';
			lua_pushnil(L);
		}
		if (use_ext)
