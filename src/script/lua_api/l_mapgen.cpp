...
int ModApiMapgen::l_register_ore(lua_State *L) {
	...
	if (get_biome_list(L, -1, bmgr, &ore->biomes))
		infostream << "register_ore: couldn't get all biomes " << '\n';
	lua_pop(L, 1);
...
int ModApiMapgen::l_generate_ores(lua_State *L) {
	...
	v3s16 pmin = lua_istable(L, 2) ? check_v3s16(L, 2) : mg.vm->m_area.MinEdge + v3s16(1, 1, 1) * MAP_BLOCKSIZE;
...
int ModApiMapgen::l_place_schematic(lua_State *L) {
	...
	if (!schem) {
		errorstream << "place_schematic: failed to get schematic" << '\n';
		return 0;
	}
...
int ModApiMapgen::l_read_schematic(lua_State *L) {
	...
	if (!schem) {
		errorstream << "read_schematic: failed to get schematic" << '\n';
		return 0;
	}
	lua_pop(L, 2);
