...
void TestModStorageDatabase::runTests(IGameDef *gamedef)
{
	...
	rawstream << "-------- Dummy database (same object only)" << '\n';
	...
	rawstream << "-------- Files database (same object)" << '\n';
	...
	rawstream << "-------- Files database (new objects)" << '\n';
	...
	rawstream << "-------- SQLite3 database (same object)" << '\n';
	...
	rawstream << "-------- SQLite3 database (new objects)" << '\n';
...
#if USE_POSTGRESQL
	const char *env_postgresql_connect_string = getenv("APEROSENGINE_POSTGRESQL_CONNECT_STRING");
	if (env_postgresql_connect_string) {
		std::string connect_string(env_postgresql_connect_string);

		rawstream << "-------- PostgreSQL database (same object)" << '\n';
		...
		rawstream << "-------- PostgreSQL database (new objects)" << '\n';
