...
void Server::handleCommand_Init(NetworkPacket* pkt)
{
	...
	if (m_simple_singleplayer_mode && !m_clients.getClientIDs().empty()) {
		infostream << "Server: Not allowing another client (" << addr_s <<
			") to connect in simple singleplayer mode" << '\n';
	...
	*pkt >> client_max >> supp_compr_modes >> min_net_proto_version
	...
	if (depl_serial_v == SER_FMT_VER_INVALID) {
		actionstream << "Server: A mismatched client tried to connect from " <<
			addr_s << " ser_fmt_max=" << (int)client_max << '\n';
	...
	*/
	...
	if (pns == 0 || pns > PLAYERNAME_SIZE) {
		actionstream << "Server: Player with " <<
			...
			" name tried to connect from " << addr_s << '\n';
	...
	if (player && player->getPeerId() != PEER_ID_INEXISTENT) {
		actionstream << "Server: Player with name \"" << playername <<
			"\" tried to connect, but player with same name is already connected" << '\n';
