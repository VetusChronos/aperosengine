...
void Server::handleCommand_Init(NetworkPacket *pkt) {
	...
	if (client->getState() > CS_Created) {
		verbosestream << "Server: Ignoring multiple TOSERVER_INITs from " << addr_s << " (peer_id=" << peer_id << ")" << '\n';
		return;
	}

	client->setCachedAddress(addr);

	verbosestream << "Server: Got TOSERVER_INIT from " << addr_s << " (peer_id=" << peer_id << ")" << '\n';

	// Do not allow multiple players in simple singleplayer mode.
	// This isn't a perfect way to do it, but will suffice for now
	if (m_simple_singleplayer_mode && !m_clients.getClientIDs().empty()) {
		infostream << "Server: Not allowing another client (" << addr_s << ") to connect in simple singleplayer mode" << '\n';
	...
	*pkt >> client_max >> supp_compr_modes >> min_net_proto_version >> max_net_proto_version >> playerName;
	...
	if (depl_serial_v == SER_FMT_VER_INVALID) {
		actionstream << "Server: A mismatched client tried to connect from " << addr_s << " ser_fmt_max=" << (int)client_max << '\n';
	...
	*/
	...
	verbosestream << "Server: " << addr_s << ": Protocol version: min: "
				  ...
				  << ", chosen: " << net_proto_version << '\n';

	client->net_proto_version = net_proto_version;

	if (net_proto_version < Server::getProtocolVersionMin() ||
			net_proto_version > Server::getProtocolVersionMax()) {
		actionstream << "Server: A mismatched client tried to connect from " << addr_s << " proto_max=" << (int)max_net_proto_version << '\n';
		DenyAccess(peer_id, SERVER_ACCESSDENIED_WRONG_VERSION);
		return;
	}
	...
	*/
	...
	if (pns == 0 || pns > PLAYERNAME_SIZE) {
		actionstream << "Server: Player with " << ((pns > PLAYERNAME_SIZE) ? "a too long" : "an empty") << " name tried to connect from " << addr_s << '\n';
	...
	if (player && player->getPeerId() != PEER_ID_INEXISTENT) {
		actionstream << "Server: Player with name \"" << playername << "\" tried to connect, but player with same name is already connected" << '\n';
	...
	infostream << "Server: New connection: \"" << playerName << "\" from " << addr_s << " (peer_id=" << peer_id << ")" << '\n';
	...
	if (m_clients.isUserLimitReached() &&
		...
		actionstream << "Server: " << playername << " tried to join from " << addr_s << ", but there are already max_users=" << g_settings->getU16("max_users") << " players." << '\n';
...
void Server::handleCommand_Init2(NetworkPacket *pkt) {
	session_t peer_id = pkt->getPeerId();
	verbosestream << "Server: Got TOSERVER_INIT2 from " << peer_id << '\n';
	...
	*/
	...
	if (client->net_proto_version < LATEST_PROTOCOL_VERSION) {
		SendChatMessage(peer_id, ChatMessage(CHATMESSAGE_TYPE_SYSTEM, L"# Server: WARNING: YOUR CLIENT'S VERSION MAY NOT BE FULLY COMPATIBLE "
																	  L"WITH THIS SERVER!"));
	}
}
...
void Server::handleCommand_PlayerPos(NetworkPacket *pkt) {
	...
	if (player == NULL) {
		errorstream << "Server::ProcessData(): Canceling: No player for peer_id=" << peer_id << " disconnecting peer!" << '\n';
		DisconnectPeer(peer_id);
		return;
	}
	...
	if (playersao == NULL) {
		errorstream << "Server::ProcessData(): Canceling: No player object for peer_id=" << peer_id << " disconnecting peer!" << '\n';
		DisconnectPeer(peer_id);
		return;
	}
...
void Server::handleCommand_Interact(NetworkPacket *pkt) {
	...
	*/
	if (!checkPriv(player->getName(), "interact")) {
		actionstream << player->getName() << " attempted to interact with " << pointed.dump() << " without 'interact' privilege" << '\n';
	...
	if ((action == INTERACT_START_DIGGING || action == INTERACT_DIGGING_COMPLETED ||
		...
		if (pointed.type == POINTEDTHING_NODE) {
			...
			if (playersao->getId() == pointed_object->getId()) {
				actionstream << "Server: " << player->getName()
							 << " attempted to interact with themselves" << '\n';
	...
	*/
	...
	switch (action) {
		...
		case INTERACT_PLACE: {
			...
			const bool had_prediction = !selected_item->getDefinition(m_itemdef).node_placement_prediction.empty();
		...
		default:
			warningstream << "Server: Invalid action " << action << '\n';
	}
}
	...
	*pkt >> client_formspec_name;
	...
	if (!pkt_read_formspec_fields(pkt, fields)) {
		warningstream << "Too large formspec fields! Ignoring for formname=\""
					  << client_formspec_name << "\", player=" << player->getName() << '\n';
		return;
	}
	...
	actionstream << ", possible exploitation attempt" << '\n';
}
...
void Server::handleCommand_FirstSrp(NetworkPacket *pkt) {
	...
	*pkt >> salt >> verification_key >> is_empty;
	...
	if (cstate == CS_HelloSent) {
		...
		if (empty_disallowed) {
			actionstream << "Server: " << playername
						 << " supplied empty password" << '\n';
			SendChatMessage(peer_id, ChatMessage(CHATMESSAGE_TYPE_SYSTEM, L"Changing to an empty password is not allowed."));
			return;
		}
		...
		if (success) {
			actionstream << playername << " changes password" << '\n';
			SendChatMessage(peer_id, ChatMessage(CHATMESSAGE_TYPE_SYSTEM, L"Password change successful."));
			client->setEncryptedPassword(encpwd);
		} else {
			actionstream << playername << " tries to change password but it fails" << '\n';
			SendChatMessage(peer_id, ChatMessage(CHATMESSAGE_TYPE_SYSTEM, L"Password change failed or unavailable."));
		}
	}
}
...
void Server::handleCommand_SrpBytesA(NetworkPacket *pkt) {
	...
	if (!((cstate == CS_HelloSent) || (cstate == CS_Active))) {
		actionstream << "Server: got SRP _A packet in wrong state " << cstate << " from " << getPeerAddress(peer_id).serializeString() << ". Ignoring." << '\n';
		return;
	}
	...
	if (client->chosen_mech != AUTH_MECHANISM_NONE) {
		actionstream << "Server: got SRP _A packet, while auth is already "
						"going on with mech "
					 << client->chosen_mech << " from " << getPeerAddress(peer_id).serializeString() << " (wantSudo=" << wantSudo << "). Ignoring." << '\n';
		if (wantSudo) {
	...
	*pkt >> bytes_A >> based_on;
	...
	AuthMechanism chosen = (based_on == 0) ? AUTH_MECHANISM_LEGACY_PASSWORD : AUTH_MECHANISM_SRP;

	if (wantSudo) {
		// Right now, the auth mechs don't change between login and sudo mode.
		if (!client->isMechAllowed(chosen)) {
			actionstream << "Server: Player \"" << client->getName() << "\" at " << getPeerAddress(peer_id).serializeString() << " tried to change password using unallowed mech " << chosen << "." << '\n';
			DenySudoAccess(peer_id);
			return;
		}
	} else {
		if (!client->isMechAllowed(chosen)) {
			actionstream << "Server: Client tried to authenticate from " << getPeerAddress(peer_id).serializeString() << " using unallowed mech " << chosen << "." << '\n';
			DenyAccess(peer_id, SERVER_ACCESSDENIED_UNEXPECTED_DATA);
			return;
		}
	}
	...
	if (based_on == 0) {
		...
		actionstream << "Server: User " << client->getName() << " tried to log in, but srp verifier field was invalid (most likely "
																"invalid base64)."
					 << '\n';
		DenyAccess(peer_id, SERVER_ACCESSDENIED_SERVER_FAIL);
		return;
...
void Server::handleCommand_SrpBytesM(NetworkPacket *pkt) {
	...
	verbosestream << "Server: Received TOSERVER_SRP_BYTES_M." << '\n';
	...
	if (client->chosen_mech != AUTH_MECHANISM_SRP &&
		...
		warningstream << "Server: got SRP_M packet, while auth "
						 "is going on with mech "
					  << client->chosen_mech << " from "
					  << addr_s << " (wantSudo=" << wantSudo << "). Denying." << '\n';
		if (wantSudo) {
			DenySudoAccess(peer_id);
			return;
		}
	...
	*pkt >> bytes_M;
	...
	if (srp_verifier_get_session_key_length((SRPVerifier *)client->auth_data) != bytes_M.size()) {
...
void Server::handleCommand_ModChannelJoin(NetworkPacket *pkt) {
	...
	*pkt >> channel_name;
	...
	if (g_settings->getBool("enable_mod_channels") &&
		...
		infostream << "Peer " << peer_id << " tried to join channel " << channel_name << ", but was already registered." << '\n';
	}
