...
void ReliablePacketBuffer::print() {
	...
	LOG(dout_con << "Dump of ReliablePacketBuffer:" << '\n');
	unsigned int index = 0;
	for (BufferedPacketPtr &packet : m_list) {
		LOG(dout_con << index << ":" << packet->getSeqnum() << '\n');
		index++;
	}
}
...
SharedBuffer<u8> IncomingSplitBuffer::insert(BufferedPacketPtr &p_ptr, bool reliable) {
	...
	if (p.size() < headersize) {
		errorstream << "Invalid data size for split packet" << '\n';
...
bool UDPPeer::processReliableSendCommand(
	...
	const u32 chunksize_max = max_packet_size - BASE_HEADER_SIZE - RELIABLE_HEADER_SIZE;
	...
	if (!have_initial_sequence_number) {
		LOG(derr_con << m_connection->getDesc() << "Ran out of sequence numbers!" << '\n');
		return false;
	}
...
session_t Connection::createPeer(const Address &sender, int fd) {
	...
	*/
	...
	if (m_peers.find(peer_id_new) != m_peers.end()) {
		errorstream << getDesc() << " ran out of peer ids" << '\n';
		return PEER_ID_INEXISTENT;
	}
	...
	LOG(dout_con << getDesc()
				 << "createPeer(): giving peer_id=" << peer_id_new << '\n');
...
void Connection::sendAck(session_t peer_id, u8 channelnum, u16 seqnum) {
	...
	LOG(dout_con << getDesc()
				 << " Queuing ACK command to peer_id: " << peer_id << " channel: " << (channelnum & 0xFF) << " seqnum: " << seqnum << '\n');
