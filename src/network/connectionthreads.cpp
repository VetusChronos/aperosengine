...
void ConnectionSendThread::runTimeouts(float dtime, u32 peer_packet_quota) {
	...
	for (const session_t peerId : peerIds) {
		...
		*/
		...
		const float peer_timeout = peer->isHalfOpen() ? MYMAX(5.0f, m_timeout / 4) : m_timeout;
		std::string reason;
		...
		for (Channel &channel : udpPeer->channels) {
			...
			if (peer->isHalfOpen()) {
				if (!timed_outs.empty()) {
					dout_con << m_connection->getDesc() << "Skipping re-send of " << timed_outs.size() << " timed-out reliables to peer_id " << udpPeer->id
							 << " (half-open)." << '\n';
				}
				continue;
			}
	...
	for (u16 timeouted_peer : timeouted_peers) {
		LOG(dout_con << m_connection->getDesc()
					 << "RunTimeouts(): Removing peer " << timeouted_peer << '\n');
...
void ConnectionSendThread::processReliableCommand(ConnectionCommandPtr &c) {
	...
	switch (c->type) {
		case CONNCMD_NONE:
			LOG(dout_con << m_connection->getDesc()
						 << "UDP processing reliable CONNCMD_NONE" << '\n');
			return;

		case CONNCMD_SEND:
			LOG(dout_con << m_connection->getDesc()
						 << "UDP processing reliable CONNCMD_SEND" << '\n');
			sendReliable(c);
			return;

		case CONNCMD_SEND_TO_ALL:
			LOG(dout_con << m_connection->getDesc()
						 << "UDP processing CONNCMD_SEND_TO_ALL" << '\n');
			sendToAllReliable(c);
			return;
		...
		case CONNCMD_RESEND_ONE: {
			LOG(dout_con << m_connection->getDesc()
						 << "UDP processing reliable CONNCMD_RESEND_ONE" << '\n');
...
void ConnectionSendThread::processNonReliableCommand(ConnectionCommandPtr &c_ptr) {
	...
	switch (c.type) {
		case CONNCMD_NONE:
			LOG(dout_con << m_connection->getDesc()
						 << " UDP processing CONNCMD_NONE" << '\n');
			return;
		case CONNCMD_SERVE:
			LOG(dout_con << m_connection->getDesc()
						 << " UDP processing CONNCMD_SERVE port="
						 << c.address.serializeString() << '\n');
			serve(c.address);
			return;
		case CONNCMD_CONNECT:
			LOG(dout_con << m_connection->getDesc()
						 << " UDP processing CONNCMD_CONNECT" << '\n');
			connect(c.address);
			return;
		case CONNCMD_DISCONNECT:
			LOG(dout_con << m_connection->getDesc()
						 << " UDP processing CONNCMD_DISCONNECT" << '\n');
			disconnect();
			return;
		case CONNCMD_DISCONNECT_PEER:
			LOG(dout_con << m_connection->getDesc()
						 << " UDP processing CONNCMD_DISCONNECT_PEER" << '\n');
			disconnect_peer(c.peer_id);
			return;
		case CONNCMD_SEND:
			LOG(dout_con << m_connection->getDesc()
						 << " UDP processing CONNCMD_SEND" << '\n');
		...
		default:
			LOG(dout_con << m_connection->getDesc()
						 << " Invalid command type: " << c.type << '\n');
	}
}

void ConnectionSendThread::serve(Address bind_address) {
	LOG(dout_con << m_connection->getDesc()
				 << "UDP serving at port " << bind_address.serializeString() << '\n');
	try {
...
void ConnectionSendThread::disconnect() {
	LOG(dout_con << m_connection->getDesc() << " disconnecting" << '\n');
...
void ConnectionSendThread::disconnect_peer(session_t peer_id) {
	LOG(dout_con << m_connection->getDesc() << " disconnecting peer" << '\n');
...
void *ConnectionReceiveThread::run() {
	...
	LOG(dout_con << m_connection->getDesc()
				 << "ConnectionReceive thread started" << '\n');
	...
	while (!stopRequested()) {
		...
		if (debug_print_timer > 20.0) {
			...
			for (auto id : peerids) {
				...
				output << "\tcurrent (sum): " << peer_current << "kb/s " << peer_loss << "kb/s" << '\n';
...
void ConnectionReceiveThread::receive(SharedBuffer<u8> &packetdata,
	...
	try {
		...
		if (channelnum >= CHANNEL_COUNT) {
			LOG(derr_con << m_connection->getDesc()
						 << "Receive(): Invalid channel " << (int)channelnum << '\n');
			return;
		}
				...
				if (++l.counter > MAX_NEW_PEERS_PER_SEC) {
					if (!l.logged) {
						warningstream << m_connection->getDesc()
									  ...
									  << " new clients within 1s. Throttling." << '\n';
					}
					l.logged = true;
		...
		try {
			...
			SharedBuffer<u8> resultdata = processPacket(channel, strippeddata, peer_id, channelnum, false);

			LOG(dout_con << m_connection->getDesc()
						 << " ProcessPacket from peer_id: " << peer_id
						 << ", channel: " << (u32)channelnum << ", returned "
						 << resultdata.getSize() << " bytes" << '\n');
...
SharedBuffer<u8> ConnectionReceiveThread::processPacket(Channel *channel,
	...
	if (!peer) {
		errorstream << "Peer not found (possible timeout)" << '\n';
...
SharedBuffer<u8> ConnectionReceiveThread::handlePacketType_Control(Channel *channel,
	...
	if (controltype == CONTROLTYPE_ACK) {
		...
		if (packetdata.getSize() < 4)
			throw InvalidIncomingDataException("packetdata.getSize() < 4 (SET_PEER_ID header size)");
		...
		// the timeout counter
		LOG(dout_con << m_connection->getDesc() << "PING" << '\n');
...
SharedBuffer<u8> ConnectionReceiveThread::handlePacketType_Original(Channel *channel,
	...
	if (packetdata.getSize() <= ORIGINAL_HEADER_SIZE)
		throw InvalidIncomingDataException("packetdata.getSize() <= ORIGINAL_HEADER_SIZE");
