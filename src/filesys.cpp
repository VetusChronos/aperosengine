...
{
	infostream << "Recursively deleting \"" << path << "\"" << '\n';
	if (!IsDir(path)) {
		infostream << "RecursiveDelete: Deleting file  " << path << '\n';
	...
	infostream << "RecursiveDelete: Deleting directory " << path << '\n';
...
bool CopyFileContents(const std::string &source, const std::string &target)
{
	...
	if (attr == INVALID_FILE_ATTRIBUTES) {
		errorstream << target << ": file disappeared after copy" << '\n';
		return false;
	}
...
{
	infostream << "Moving \"" << source << "\" to \"" << target << "\"" << '\n';
	...
	if (fs::PathExists(target)) {
		if (rmdir(target.c_str()) != 0) {
			errorstream << "MoveDir: target \"" << target
				<< "\" exists as file or non-empty folder" << '\n';
			return false;
		}
	}
...
bool safeWriteToFile(const std::string &path, std::string_view content)
{
	...
	if (handle == INVALID_HANDLE_VALUE) {
		errorstream << "Failed to open file: " << LAST_OS_ERROR() << '\n';
		return false;
	}
	DWORD bytes_written;
	...
	if (!write_error.empty()) {
		errorstream << "Failed to write file: " << write_error << '\n';
	...
	if (!rename_error.empty()) {
		errorstream << "Failed to overwrite \"" << path << "\": " << rename_error << '\n';
		remove(tmp_file.c_str());
		return false;
	}
