...
static bool read_config_file(const Settings &cmd_args)
{
	...
	if (cmd_args.exists("config")) {
		...
		filenames.push_back(porting::path_user + DIR_DELIM + "aperosengine.conf");
		// Legacy configuration file location
		filenames.push_back(porting::path_user +
				DIR_DELIM + ".." + DIR_DELIM + "aperosengine.conf");

#if RUN_IN_PLACE
		// Try also from a lower level (to aid having the same configuration
		// for many RUN_IN_PLACE installs)
		filenames.push_back(porting::path_user +
				DIR_DELIM + ".." + DIR_DELIM + ".." + DIR_DELIM + "aperosengine.conf");
#endif
...
static std::string get_clean_world_path(const std::string &path)
{
	...
	if (path.size() > worldmt.size()
		...
		dstream << _("Supplied world.apr file - stripping it off.") << '\n';
