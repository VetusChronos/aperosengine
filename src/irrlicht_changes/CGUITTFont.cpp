...
struct SGUITTFace : public virtual irr::IReferenceCounted {
	SGUITTFace() :
			face_buffer(0), face_buffer_size(0) {
...
CGUITTFont::CGUITTFont(IGUIEnvironment *env) :
		use_monochrome(false), use_transparency(true), use_hinting(true), use_auto_hinting(true), batch_load_size(1), Device(0), Environment(env), Driver(0), GlobalKerningWidth(0), GlobalKerningHeight(0), shadow_offset(0), shadow_alpha(0), fallback(0) {
#ifdef _DEBUG
...
bool CGUITTFont::load(const io::path &filename, const u32 size, const bool antialias, const bool transparency) {
	...
	if (node == c_faces.end()) {
		...
		if (filesystem) {
			...
			if (file == 0) {
				if (logger)
					logger->log("CGUITTFont", "Failed to open the file.", irr::ELL_INFORMATION);
			...
			if (FT_New_Memory_Face(c_library, face->face_buffer, face->face_buffer_size, 0, &face->face)) {
				if (logger)
					logger->log("CGUITTFont", "FT_New_Memory_Face failed.", irr::ELL_INFORMATION);
			...
			if (FT_New_Face(c_library, reinterpret_cast<const char *>(filename.c_str()), 0, &face->face)) {
				if (logger)
					logger->log("CGUITTFont", "FT_New_Face failed.", irr::ELL_INFORMATION);
	...
	**/
	...
	while (*text) {
		...
		if (line_break) {
			...
			// Note that fallback font handling is missing here (AperosEngine never uses this)
