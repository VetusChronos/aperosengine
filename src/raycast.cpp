...
bool RaycastSort::operator()(const PointedThing &pt1,
	...
	if (pt1_distSq == pt2.distanceSq) {
		...
		return (pt2.type == POINTEDTHING_OBJECT || pt1.node_undersurface < pt2.node_undersurface);
	}
	return true;
}
...
bool boxLineCollision(const aabb3f &box, const v3f start,
	...
	if (dir.X != 0) {
		...
		if (m >= 0 && m <= 1) {
			*collision_point = start + dir * m;
			if ((collision_point->Y >= box.MinEdge.Y) && (collision_point->Y <= box.MaxEdge.Y) && (collision_point->Z >= box.MinEdge.Z) && (collision_point->Z <= box.MaxEdge.Z)) {
	...
	if (dir.Y != 0) {
		...
		if (m >= 0 && m <= 1) {
			*collision_point = start + dir * m;
			if ((collision_point->X >= box.MinEdge.X) && (collision_point->X <= box.MaxEdge.X) && (collision_point->Z >= box.MinEdge.Z) && (collision_point->Z <= box.MaxEdge.Z)) {
	...
	if (dir.Z != 0) {
		...
		if (m >= 0 && m <= 1) {
			*collision_point = start + dir * m;
			if ((collision_point->X >= box.MinEdge.X) && (collision_point->X <= box.MaxEdge.X) && (collision_point->Y >= box.MinEdge.Y) && (collision_point->Y <= box.MaxEdge.Y)) {
