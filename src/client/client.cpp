...
void Client::migrateModStorage() {
	...
	if (fs::IsDir(old_mod_storage)) {
		...
		infostream << "Finished migration of client mod storage" << '\n';
	}
}
...
void Client::loadMods() {
	...
	if (!modconf.isConsistent()) {
		errorstream << modconf.getUnsatisfiedModsError() << '\n';
		return;
	}
...
void Client::Stop() {
	...
	if (m_localdb) {
		infostream << "Local map saving ended." << '\n';
		m_localdb->endSave();
	}
...
void Client::connect(const Address &address, const std::string &address_name,
	...
	if (m_con) {
		...
		infostream << "Client connection will be recreated" << '\n';
...
void Client::step(float dtime) {
	...
	*/
	...
	if (counter >= 10) {
		...
		infostream << "Client: avg_rtt=" << avg_rtt << '\n';
	}
...
void Client::initLocalMapSaving(const Address &address,
	...
	if (m_localdb) {
		infostream << "Local map saving already running" << '\n';
		return;
	}
	...
	*/
	if (pkt->getPeerId() != PEER_ID_SERVER) {
		infostream << "Client::ProcessData(): Discarding data not "
				   ...
				   << " command=" << static_cast<unsigned>(command) << '\n';
		return;
	}
...
void Client::typeChatMessage(const std::wstring &message) {
	...
	infostream << "Typed chat message: \"" << message_utf8 << "\"" << '\n';
...
u16 Client::allocateUnknownNodeId(const std::string &name) {
	errorstream << "Client::allocateUnknownNodeId(): "
				<< "Client cannot allocate node IDs" << '\n';
