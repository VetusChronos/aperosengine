...
void Sky::setSunTexture(const std::string &sun_texture,
	...
	m_sun_tonemap = tsrc->isKnownSourceImage(sun_tonemap) ? tsrc->getTexture(sun_tonemap) : nullptr;
	...
	if (sun_texture == "sun.png") {
		...
		m_sun_texture = tsrc->isKnownSourceImage(sun_texture) ? tsrc->getTexture(sun_texture) : nullptr;
...
void Sky::setSunriseTexture(const std::string &sunglow_texture,
	...
	m_materials[2].setTexture(0, tsrc->getTextureForMesh(sunglow_texture.empty() ? "sunrisebg.png" : sunglow_texture));
}
...
void Sky::setMoonTexture(const std::string &moon_texture,
	...
	m_moon_tonemap = tsrc->isKnownSourceImage(moon_tonemap) ? tsrc->getTexture(moon_tonemap) : nullptr;
	...
	if (moon_texture == "moon.png") {
		...
		m_moon_texture = tsrc->isKnownSourceImage(moon_texture) ? tsrc->getTexture(moon_texture) : nullptr;
