...
void Camera::drawWieldedTool(irr::core::matrix4 *translation) {
	...
	if (translation != NULL) {
		...
		irr::core::vector3df focusPoint = (cam->getTarget() - cam->getAbsolutePosition()).setLength(1) + cam->getAbsolutePosition();
	...
	for (const Nametag *nametag : m_nametags) {
		...
		if (transformed_pos[3] > 0) {
			...
			screen_pos.X = screensize.X *
							(0.5 * transformed_pos[0] * zDiv + 0.5) -
					textsize.Width / 2;
			screen_pos.Y = screensize.Y *
							(0.5 - transformed_pos[1] * zDiv * 0.5) -
					textsize.Height / 2;
