...
void MinimapUpdateThread::doUpdate() {
	...
	if (data->map_invalidated && (data->mode.type == MINIMAP_TYPE_RADAR || data->mode.type == MINIMAP_TYPE_SURFACE)) {
		...
		case MINIMAP_TYPE_TEXTURE:
			...
			image->copyTo(map_image,
					irr::core::vector2d<int>{
							((data->mode.map_size - (static_cast<int>(dim.Width))) >> 1) - data->pos.X / data->mode.scale,
							((data->mode.map_size - (static_cast<int>(dim.Height))) >> 1) + data->pos.Z / data->mode.scale });
			image->drop();
	}
...
void Minimap::drawMinimap(core::rect<s32> rect) {
	...
	// Draw overlay
	video::ITexture *minimap_overlay = data->minimap_shape_round ? data->minimap_overlay_round : data->minimap_overlay_square;
...
void Minimap::updateActiveMarkers() {
	...
	v3s16 pos_offset = data->pos - v3s16(data->mode.map_size / 2, data->mode.scan_height / 2, data->mode.map_size / 2);
