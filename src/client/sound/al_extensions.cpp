...
ALExtensions::ALExtensions(const ALCdevice *deviceHandle [[maybe_unused]])
{
...

	{
		...
		if (blacklisted)
			infostream << "ALExtensions: Blacklisted: " << ext_name << '\n';
#ifndef AL_SOFT_direct_channels_remix
		infostream << "ALExtensions: Not compiled with: " << ext_name << '\n';
#else
		bool found = alIsExtensionPresent(ext_name);
		if (found)
			infostream << "ALExtensions: Found: " << ext_name << '\n';
		else
			infostream << "ALExtensions: Not found: " << ext_name << '\n';
