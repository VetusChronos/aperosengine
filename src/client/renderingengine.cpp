...
static irr::IrrlichtDevice *createDevice(SIrrlichtCreationParameters params, std::optional<video::E_DRIVER_TYPE> requested_driver)
{
	if (requested_driver) {
		...
		verbosestream << "Trying video driver " << getVideoDriverName(params.DriverType) << '\n';
		if (auto *device = createDeviceEx(params))
			return device;
		errorstream << "Failed to initialize the " << getVideoDriverName(params.DriverType) << " video driver" << '\n';
	}
...
RenderingEngine::RenderingEngine(IEventReceiver *receiver)
{
	...
	verbosestream << "Using the " << getVideoDriverName(driver->getDriverType()) << " video driver" << '\n';
	...
	auto skin = createSkin(m_device->getGUIEnvironment(), gui::EGST_WINDOWS_CLASSIC, driver);
