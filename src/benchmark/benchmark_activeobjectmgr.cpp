...
public:
	TestObject(v3f pos) :
			ServerActiveObject(nullptr, pos) {}
...
#define BENCH_INSIDE_RADIUS(_count)              \
	BENCHMARK_ADVANCED("inside_radius_" #_count) \
	(Catch::Benchmark::Chronometer meter) { benchGetObjectsInsideRadius<_count>(meter); };

#define BENCH_IN_AREA(_count)              \
	BENCHMARK_ADVANCED("in_area_" #_count) \
	(Catch::Benchmark::Chronometer meter) { benchGetObjectsInArea<_count>(meter); };
