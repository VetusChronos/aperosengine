...
GUIChatConsole::GUIChatConsole(
	...
	m_font = g_fontengine->getFont(chat_font_size != 0 ? rangelim(chat_font_size, 5, 72) : FONT_SIZE_UNSPECIFIED, FM_Mono);
	...
	if (event.EventType == EET_KEY_INPUT_EVENT && !event.KeyInput.PressedDown) {
		...
		if (event.KeyInput.Key == KEY_ESCAPE) {
			...
			if (!has_char) { // no num lock
				...
				ChatPrompt::CursorOp op = event.KeyInput.Shift ? ChatPrompt::CURSOROP_SELECT : ChatPrompt::CURSOROP_MOVE;
				ChatPrompt::CursorOpDir dir = event.KeyInput.Key == KEY_LEFT ? ChatPrompt::CURSOROP_DIR_LEFT : ChatPrompt::CURSOROP_DIR_RIGHT;
				ChatPrompt::CursorOpScope scope = event.KeyInput.Control ? ChatPrompt::CURSOROP_SCOPE_WORD : ChatPrompt::CURSOROP_SCOPE_CHARACTER;
			...
			ChatPrompt::CursorOpScope scope =
					event.KeyInput.Control ? ChatPrompt::CURSOROP_SCOPE_WORD : ChatPrompt::CURSOROP_SCOPE_CHARACTER;
			prompt.cursorOperation(
		...
		// Otherwise, middle click pastes primary selection
			...
			if (event.MouseInput.Y / m_fontsize.Y < (m_height / m_fontsize.Y) - 1) {
				...
				if (!was_url_pressed && event.MouseInput.Event == EMIE_MMOUSE_PRESSED_DOWN) {
