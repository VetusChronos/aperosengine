	...
	m_kill(kill)
{
	...
	infostream << "GUIEngine: Initializing Lua" << '\n';
	...
	try {
		...
		if (!loadMainMenuScript()) {
			errorstream << "No future without main menu!" << '\n';
			abort();
		}

		run();
	} catch (LuaError &e) {
		errorstream << "Main menu error: " << e.what() << '\n';
...
GUIEngine::~GUIEngine()
{
	...
	infostream << "GUIEngine: Deinitializing scripting" << '\n';
	m_script.reset();
