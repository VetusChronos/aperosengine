...
void update_lighting_nodes(Map *map,
	...
	for (LightBank bank : banks) {
		...
		for (auto it = oldnodes.cbegin(); it < oldnodes.cend(); ++it) {
			...
			if (f.light_propagates) {
				if (bank == LIGHTBANK_DAY && f.sunlight_propagates && is_sunlight_above(map, p, ndef)) {
					new_light = LIGHT_SUN;
				} else {
	...
	for (index = 0; index < data->data.size(); index++) {
		...
		if (it.is_sunlit) {
			...
			for (; current_pos.Y >= 0; current_pos.Y--) {
				...
				ContentLightingFlags f = ndef->getLightingFlags(n);
				if (n.getLightRaw(LIGHTBANK_DAY, f) < LIGHT_SUN && f.sunlight_propagates) {
					...
					for (relpos.X = 0; relpos.X < MAP_BLOCKSIZE; relpos.X++)
						for (relpos.Y = 0; relpos.Y < MAP_BLOCKSIZE; relpos.Y++) {
							...
							for (size_t b = 0; b < 2; b++) {
								...
								u8 light = f.has_light ? node.getLight(bank, f) : f.light_source;
								if (light > 1)
					...
					for (relpos.Z = a.MinEdge.Z; relpos.Z <= a.MaxEdge.Z; relpos.Z++)
						for (relpos.X = a.MinEdge.X; relpos.X <= a.MaxEdge.X; relpos.X++)
							for (relpos.Y = a.MinEdge.Y; relpos.Y <= a.MaxEdge.Y; relpos.Y++) {
								...
								for (size_t b = 0; b < 2; b++) {
									LightBank bank = banks[b];
									u8 oldlight = oldf.has_light ? oldnode.getLight(bank, oldf) : LIGHT_SUN; // no light information, force unlighting
									u8 newlight = newf.has_light ? newnode.getLight(bank, newf) : newf.light_source;
...
VoxelLineIterator::VoxelLineIterator(const v3f &start_position, const v3f &line_vector) :
	...
	if (m_line_vector.X > 0) {
		m_next_intersection_multi.X = (m_current_node_pos.X + 0.5f - m_start_position.X) / m_line_vector.X;
	...
	if (m_line_vector.Y > 0) {
		m_next_intersection_multi.Y = (m_current_node_pos.Y + 0.5f - m_start_position.Y) / m_line_vector.Y;
	...
	if (m_line_vector.Z > 0) {
		m_next_intersection_multi.Z = (m_current_node_pos.Z + 0.5f - m_start_position.Z) / m_line_vector.Z;
...
void VoxelLineIterator::next() {
	m_current_index++;
	if ((m_next_intersection_multi.X < m_next_intersection_multi.Y) && (m_next_intersection_multi.X < m_next_intersection_multi.Z)) {
...
s16 VoxelLineIterator::getIndex(v3s16 voxel) {
	return abs(voxel.X - m_start_node_pos.X) +
