...
void NodeTimerList::serialize(std::ostream &os, u8 map_format_version) const {
	if (map_format_version == 24) {
		...
		writeU16(os, static_cast<u16>(m_timers.size()));
	}
	...
	for (const auto &timer : m_timers) {
		const auto &t = timer.second;
		NodeTimer nt(t.timeout, t.timeout - static_cast<f32>(timer.first - m_time), t.position);
		v3s16 p = t.position;
...
void NodeTimerList::deSerialize(std::istream &is, u8 map_format_version) {
	...
	for (u16 i = 0; i < count; ++i) {
		...
		p16 %= MAP_BLOCKSIZE * MAP_BLOCKSIZE;
		p.Y = p16 / MAP_BLOCKSIZE;
		p.X = p16 % MAP_BLOCKSIZE;
...
std::vector<NodeTimer> NodeTimerList::step(float dtime) {
	...
	m_time += dtime;
	if (m_next_trigger_time == -1.0 || m_time < m_next_trigger_time) {
		return elapsed_timers;
	}

	auto it = m_timers.begin();
	for (; it != m_timers.end() && it->first <= m_time; ++it) {
		NodeTimer t = it->second;
		t.elapsed = t.timeout + static_cast<f32>(m_time - it->first);
