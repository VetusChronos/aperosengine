...
struct Parameter {
	...
	virtual void serialize(std::ostream &os) const { serializeParameterValue(os, this->val); }
	virtual void deSerialize(std::istream &is) { deSerializeParameterValue(is, this->val); }
...
enum class AttractorKind : u8 { none,
	point,
	line,
	plane };
enum class BlendMode : u8 { alpha,
	add,
	sub,
	screen };
