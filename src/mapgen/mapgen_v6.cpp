...
Copyright (C) 2013-2018 kwolekr, Ryan Kwolek <kwolekr@aperosvoxel.domain>
...
void MapgenV6Params::setDefaultSettings(Settings *settings) {
	settings->setDefault("mgv6_spflags", flagdesc_mapgen_v6, MGV6_JUNGLES | MGV6_SNOWBIOMES | MGV6_TREES | MGV6_BIOMEBLEND | MGV6_MUDFLOW | MGV6_TEMPLES);
}
...
BiomeV6Type MapgenV6::getBiome(v2s16 p) {
	int index = (p.Y - full_node_min.Z) * (ystride + 2 * MAP_BLOCKSIZE) + (p.X - full_node_min.X);
...
void MapgenV6::makeChunk(BlockMakeData *data) {
	...
	if ((flags & MG_DUNGEONS) && stone_surface_max_y >= node_min.Y &&
		...
		u16 num_dungeons = std::fmax(std::floor(
											 NoisePerlin3D(&np_dungeons, node_min.X, node_min.Y, node_min.Z, seed)),
				0.0f);
		...
		for (s16 x = node_min.X; x <= node_max.X; x++, index++) {
			...
			for (s16 y = node_min.Y; y <= node_max.Y; y++) {
				if (vm->m_data[i].getContent() == CONTENT_IGNORE) {
					if (y <= surface_y) {
						vm->m_data[i] = (y >= MGV6_DESERT_STONE_BASE && bt == BT_DESERT) ? n_desert_stone : n_stone;
					} else if (y <= water_level) {
						vm->m_data[i] = (y >= MGV6_ICE_BASE && bt == BT_TUNDRA) ? n_ice : n_water_source;
					} else {
						vm->m_data[i] = n_air;
					}
				}
