...
Copyright (C) 2014-2016 kwolekr, Ryan Kwolek <kwolekr@aperosvoxel.domain>
...
void OreSheet::generate(MMVManip *vm, int mapseed, u32 blockseed,
	...
	int y_start = y_start_min < y_start_max ? pr.range(y_start_min, y_start_max) : (y_start_min + y_start_max) / 2;
...
void OreStratum::generate(MMVManip *vm, int mapseed, u32 blockseed,
	...
	for (int z = nmin.Z; z <= nmax.Z; z++)
		for (int x = nmin.X; x <= nmax.X; x++, index++) {
			...
			if (flags & OREFLAG_USE_NOISE) {
				float nhalfthick = ((flags & OREFLAG_USE_NOISE2) ? noise_stratum_thickness->result[index] : (float)stratum_thickness) /
						2.0f;
