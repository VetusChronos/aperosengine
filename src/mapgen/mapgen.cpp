		...
		for (s16 x = nmin.X + 1; x <= nmax.X - 1; x++) {
			...
			for (s16 y = nmax.Y; y >= nmin.Y; y--) {
				...
				if (isignored || wasignored || isliquid == wasliquid) {
					...
					if (!waspushed && (ndef->get(vm->m_data[vi]).floodable || (!waschecked && isLiquidHorizontallyFlowable(vi_above, em)))) {
...
void MapgenBasic::generateDungeons(s16 max_stone_y) {
	...
	if (biome->c_dungeon != CONTENT_IGNORE) {
		...
		dp.c_stair = (biome->c_dungeon_stair != CONTENT_IGNORE) ? biome->c_dungeon_stair : biome->c_dungeon;
...
void GenerateNotifier::getEvents(
	...
	for (auto &gn : m_notify_events) {
		...
		std::string name = (gn.type == GENNOTIFY_DECORATION) ? "decoration#" + itos(gn.id) : flagdesc_gennotify[gn.type].name;
