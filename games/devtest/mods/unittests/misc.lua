...
local function test_v3s16_metatable(player, pos)
	...
	local found_pos = aperosengine.find_node_near(pos, 0, node.name, true)
...
local function test_game_info()
	local info = aperosengine.get_game_info()
...
local function test_mapgen_edges(cb)
	...
	local min_edge, max_edge = aperosengine.get_mapgen_edges()
	local min_finished = {}
	local max_finished = {}
	local function finish()
		...
		if min_finished[1] ~= (min_edge / aperosengine.MAP_BLOCKSIZE):floor() then
		...
		if max_finished[1] ~= (max_edge / aperosengine.MAP_BLOCKSIZE):floor() then
	...
	local function emerge_block(blockpos, action, blocks_left, finished)
		if action ~= aperosengine.EMERGE_CANCELLED then
	...
	aperosengine.emerge_area(min_edge:subtract(1), min_edge, emerge_block, min_finished)
	aperosengine.emerge_area(max_edge, max_edge:add(1), emerge_block, max_finished)
end
...
local function test_on_mapblocks_changed(cb, player, pos)
	local bp1 = (pos / aperosengine.MAP_BLOCKSIZE):floor()
	local bp2 = bp1:add(1)
	for _, bp in ipairs({bp1, bp2}) do
		-- Make a modification in the block.
		local p = bp * aperosengine.MAP_BLOCKSIZE
		aperosengine.load_area(p)
		local meta = aperosengine.get_meta(p)
	...
	finish_test_on_mapblocks_changed = function(modified_blocks, modified_block_count)
		...
		if not modified_blocks[aperosengine.hash_node_position(bp1)] or
				not modified_blocks[aperosengine.hash_node_position(bp2)] then
