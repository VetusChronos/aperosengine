...
local font_states = {
	...
	{7, "Colors test! " .. aperosengine.colorize("green", "Green") ..
		aperosengine.colorize("red", "\nRed") .. " END"},
}
...
aperosengine.register_globalstep(function(dtime)
	...
	font_etime = 0
	for _, player in ipairs(aperosengine.get_connected_players()) do
...
aperosengine.register_chatcommand("hudfonts", {
	params = "[<HUD elements>]",
	...
	func = function(name, param)
		...
		if player_font_huds[name] == nil then
			...
			aperosengine.chat_send_player(name, ("%d text HUD element(s) added."):format(param))
		else
			...
			if huds then
				...
				aperosengine.chat_send_player(name, "All text HUD elements removed.")
			end
...
aperosengine.register_chatcommand("hudwaypoints", {
	...
	func = function(name, params)
		...
		local function change(chplayer)
			...
			aperosengine.chat_send_player(chplayer:get_player_name(), "Waypoints changed.")
		end
		if params == "add_change" then
			-- change immediate
			change(player)
		else
			aperosengine.after(0.5, change, player)
		end
...
aperosengine.register_chatcommand("zoomfov", {
	params = "[<FOV>]",
...
aperosengine.register_chatcommand("hudhotbars", {
...
aperosengine.register_chatcommand("hudprint", {
...
aperosengine.register_chatcommand("hudtoggleflag", {
