...
local function show_formspec(pos, player)
	...
	local pos_hash = aperosengine.hash_node_position(pos)
	...
	aperosengine.show_formspec(player:get_player_name(), "soundstuff:jukebox@"..pos:to_string(),
			table.concat(fs))
end

aperosengine.register_node("soundstuff:jukebox", {
...
aperosengine.register_on_player_receive_fields(function(player, formname, fields)
	...
	if not pos or pos ~= pos:round() then
		aperosengine.log("error", "[soundstuff:jukebox] Invalid formname.")
		return true
	end
	...
	if not sound_handle then
		if fields.btn_play then
			...
			local sparam = {
				...
				loop = aperosengine.is_yes(md.sparam.loop),
			...
			local ephemeral = aperosengine.is_yes(md.ephemeral)

			log_msg(string.format(
					"[soundstuff:jukebox] Playing sound: aperosengine.sound_play(%s, %s, %s)",
			...
			sound_handle = try_call(aperosengine.sound_play, sss, sparam, ephemeral)
		...
		if fields.btn_stop then
			log_msg("[soundstuff:jukebox] Stopping sound: aperosengine.sound_stop(<handle>)")

			try_call(aperosengine.sound_stop, sound_handle)
		...
		elseif fields.btn_fade then
			...
			log_msg(string.format(
					"[soundstuff:jukebox] Fading sound: aperosengine.sound_fade(<handle>, %s, %s)",
					step, gain))

			try_call(aperosengine.sound_fade, sound_handle, step, gain)
		end
	end
