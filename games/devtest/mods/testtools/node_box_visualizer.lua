...
aperosengine.register_entity("testtools:visual_box", {
	initial_properties = {
		visual = "cube",
		textures = {
	...
	on_activate = function(self)
		self.timestamp = aperosengine.get_us_time() + 5000000
	end,

	on_step = function(self)
		if aperosengine.get_us_time() >= self.timestamp then
			self.object:remove()
		end
	end,
})
...
local function visualizer_on_use(itemstack, user, pointed_thing)
	...
	local result = aperosengine.get_node_boxes(box_type, pointed_thing.under)
	...
	for _, box in ipairs(result) do
		...
		local obj = aperosengine.add_entity(box_center, "testtools:visual_box")
		if not obj then
			break
		end
		obj:set_properties({
...
local function visualizer_on_place(itemstack, placer, pointed_thing)
	...
	aperosengine.chat_send_player(placer:get_player_name(), S("[Node Box Visualizer] box_type = @1", new_value))

	return itemstack
end

aperosengine.register_tool("testtools:node_box_visualizer", {
