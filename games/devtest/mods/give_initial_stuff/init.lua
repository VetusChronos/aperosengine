...
local give_initial_stuff = function(player)
	...
	aperosengine.log("action", "[give_initial_stuff] Giving initial stuff to "..player:get_player_name())
end

aperosengine.register_on_newplayer(function(player)
	if aperosengine.settings:get_bool("give_initial_stuff", true) then
		give_initial_stuff(player)
	end
end)

aperosengine.register_chatcommand("stuff", {
	params = "",
	privs = { give = true },
