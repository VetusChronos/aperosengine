...
local function bench_name2content()
	...
	local get_content_id = aperosengine.get_content_id

	local start = aperosengine.get_us_time()

	for i = 1, 200 do
		for name in pairs(aperosengine.registered_nodes) do
			t[#t + 1] = get_content_id(name)
		end
	end

	local finish = aperosengine.get_us_time()
...
local function bench_content2name()
	...
	local n = 0
	for _ in pairs(aperosengine.registered_nodes) do
		n = n + 1
	end

	local get_name_from_content_id = aperosengine.get_name_from_content_id
...
aperosengine.register_chatcommand("bench_name2content", {
	params = "",
	description = "Benchmark: Conversion from node names to content IDs",
	func = function(name, param)
		aperosengine.chat_send_player(name, "Benchmarking aperosengine.get_content_id. Warming up ...")
		bench_name2content()
...
aperosengine.register_chatcommand("bench_content2name", {
	params = "",
	description = "Benchmark: Conversion from content IDs to node names",
	func = function(name, param)
		aperosengine.chat_send_player(name, "Benchmarking aperosengine.get_name_from_content_id. Warming up ...")
		bench_content2name()
		aperosengine.chat_send_player(name, "Warming up finished, now benchmarking ...")
...
aperosengine.register_chatcommand("bench_bulk_set_node", {
	params = "",
	...
	func = function(name, param)
		...
		aperosengine.chat_send_player(name, "Benchmarking aperosengine.bulk_set_node. Warming up ...")

		-- warm up with stone to prevent having different callbacks
		-- due to different node topology
		aperosengine.bulk_set_node(pos_list, {name = "mapgen_stone"})

		aperosengine.chat_send_player(name, "Warming up finished, now benchmarking ...")

		local start_time = aperosengine.get_us_time()
		for i=1,#pos_list do
			aperosengine.set_node(pos_list[i], {name = "mapgen_stone"})
		end
		local middle_time = aperosengine.get_us_time()
		aperosengine.bulk_set_node(pos_list, {name = "mapgen_stone"})
		local end_time = aperosengine.get_us_time()
		local msg = string.format("Benchmark results: aperosengine.set_node loop: %.2f ms; aperosengine.bulk_set_node: %.2f ms",
...
aperosengine.register_chatcommand("bench_bulk_get_node", {
	params = "",
	...
	func = function(name, param)
		...
		local function bench()
			local start_time = aperosengine.get_us_time()
			for i=1,#pos_list do
				local n = aperosengine.get_node(pos_list[i])
			...
			return aperosengine.get_us_time() - start_time
		end

		aperosengine.chat_send_player(name, "Benchmarking aperosengine.get_node. Warming up ...")
		bench()

		aperosengine.chat_send_player(name, "Warming up finished, now benchmarking ...")
		local result_us = bench()

		local msg = string.format("Benchmark results: aperosengine.get_node loop 1: %.2f ms",
				result_us / 1000)
		return true, msg
	end,
})
