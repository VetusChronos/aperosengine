...
local spos = function(self)
	return aperosengine.pos_to_string(vector.round(self.object:get_pos()))
end

-- Callback test entity (all callbacks except on_step)
aperosengine.register_entity("callbacks:callback", {
	initial_properties = {
...
aperosengine.register_entity("callbacks:callback_step", {
	visual = "upright_sprite",
...
aperosengine.register_entity("callbacks:callback_puncher", {
	initial_properties = {
		visual = "upright_sprite",
		textures = { "callbacks_callback_entity.png" },
