local color = aperosengine.colorize

-- \208\176 is a cyrillic small a
local unsafe_url = aperosengine.formspec_escape("https://u:p@wikipedi\208\176.org:1233/heIIoll?a=b#c")
...
local hypertext_fs = "hypertext[0,0;11,9;hypertext;"..aperosengine.formspec_escape(hypertext_basic).."]"..
	"hypertext[0,9.5;11,2.5;hypertext;"..aperosengine.formspec_escape(hypertext_global).."]"
...
local style_fs = [[
	...
	textarea[0,7.025;2.5,0.8;one_f3;Label;]] ..
		aperosengine.formspec_escape("Yellow Text\nLine two") .. [[ ]

	style[one_f4;border=false;textcolor=cyan]
	textarea[0,8.324999999999999;2.5,0.8;one_f4;Label;]] ..
		aperosengine.formspec_escape("Borderless Cyan Text\nLine two") .. [[ ]
...
local function show_test_formspec(pname)
	...
	aperosengine.show_formspec(pname, "testformspec:formspec", fs)
end
...
aperosengine.register_on_player_receive_fields(function(player, formname, fields)
	...
	if fields.ani_img_1 and fields.ani_btn_1 then
		aperosengine.chat_send_player(player:get_player_name(), "ani_img_1 = " .. tostring(fields.ani_img_1))
		return true
	elseif fields.ani_img_2 and fields.ani_btn_2 then
		aperosengine.chat_send_player(player:get_player_name(), "ani_img_2 = " .. tostring(fields.ani_img_2))
		return true
	end

	if fields.hypertext then
		aperosengine.chat_send_player(player:get_player_name(), "Hypertext action received: " .. tostring(fields.hypertext))
		return true
	end
...
aperosengine.register_chatcommand("test_formspec", {
	params = "",
	description = "Open the test formspec",
	func = function(name)
		if not aperosengine.get_player_by_name(name) then
