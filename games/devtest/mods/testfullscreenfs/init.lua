...
local function show_fullscreen_fs(name, window)
	...
	aperosengine.show_formspec(name, "testfullscreenfs:fs", table.concat(fs))
	aperosengine.chat_send_player(name, ("Calculated size of %f, %f"):format(size.x, size.y))
	last_window_info[name] = window
end

aperosengine.register_chatcommand("testfullscreenfs", {
	func = function(name)
		local window = aperosengine.get_player_window_information(name)
		if not window then
...
aperosengine.register_globalstep(function()
