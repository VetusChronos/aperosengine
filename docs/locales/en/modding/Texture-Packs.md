AperosEngine Texture Pack Reference
===================================
...
* `name`: The texture pack name. Allows AperosEngine to determine the texture pack name even if
...
the apple texture of AperosVoxel, add a PNG file named `default_apple.png` or similar.
...
* `moon.png`: texture of the moon. Default texture is generated by AperosEngine
* `moon_tonemap.png`: tonemap to be used when `moon.png` was found
* `sun.png`: texture of the sun. Default texture is generated by AperosEngine
...
AperosEngine has three modes for rendering leaves nodes if the node has the
