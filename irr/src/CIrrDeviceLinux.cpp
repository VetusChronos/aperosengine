...
void CIrrDeviceLinux::setupTopLevelXorgWindow()
{
	...
	// Note: SDL uses the executable name here (i.e. "aperosengine").
	XClassHint *classhint = XAllocClassHint();
	classhint->res_name = const_cast<char *>("AperosEngine");
	classhint->res_class = const_cast<char *>("AperosEngine");
	...
	// the _NET_WM_PID (see further down) but running AperosEngine in an env
	// where the window manager is on another machine from AperosEngine (therefore
	...
	// Set the WM_CLIENT_LEADER window property here. AperosEngine has only one
